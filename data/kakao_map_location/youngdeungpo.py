...
def get_cloth_csv_data(read_filename, write_filename, missed_file_name):
    ...
    for line in reader:
        ...
        if not response.json()['documents']:
            missed_count += 1
            if line[3] == 'NaN':
                missed_writer.writerow(['영등포구', ' '.join(line[1:3])])
            else:
                missed_writer.writerow(['영등포구', ' '.join(line[1:4])])
            continue
        response_result = response.json()['documents'][0]
        row = [response_result['address_name'], response_result['x'], response_result['y']]
        if line[3] == 'NaN':
            row.append(' '.join(line[1:3]))
        else:
            row.append(' '.join(line[1:4]))
        writer.writerow(row)
        success_count += 1
    rf.close()
    wf.close()
...
def get_lamp_battery_csv_data(read_filename, write_filename, missed_file_name):
    ...
    for line in reader:
        ...
        if not response.json()['documents']:
            missed_count += 1
            missed_writer.writerow(['영등포구', line[-1]])
            continue
        response_result = response.json()['documents'][0]
        row = [response_result['address_name'], response_result['x'], response_result['y']]
        writer.writerow(row)
