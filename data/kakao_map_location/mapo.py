...
def get_cloth_csv_data(read_filename, write_filename, missed_file_name):
    ...
    for line in reader:
        ...
        if not response.json()['documents']:
            missed_count += 1
            if line[2] == '주소없음!':
                missed_writer.writerow(['마포구', line[1]])
            else:
                missed_writer.writerow(['마포구', ' '.join(line[1:3])])
            continue
...
def get_lamp_battery_csv_data(read_filename, write_filename, missed_file_name):
    ...
    for line in reader:
        ...
        if not response.json()['documents']:
            ...
            continue
        response_result = response.json()['documents'][0]
        writer.writerow([response_result['address_name'], response_result['x'], response_result['y'], line[2]])
        success_count += 1
    rf.close()
    wf.close()
    missed_f.close()
