...
def get_lamp_battery_csv_data(read_filename, write_filename, missed_file_name):
    ...
    for line in reader:
        ...
        if not response.json()['documents']:
            missed_count += 1
            missed_writer.writerow(['관악구', ' '.join(line[2:4])])
            continue
        response_result = response.json()['documents'][0]
        row = [response_result['address_name'], response_result['address']['x'], response_result['address']['y'], line[3]]
        writer.writerow(row)
        success_count += 1
    rf.close()
    wf.close()
